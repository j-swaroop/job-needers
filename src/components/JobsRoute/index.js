...
class Jobs extends Component {
  ...
  renderJobsDetails = () => {
    const {jobDetails, locationList} = this.state

    if (jobDetails.length !== 0) {
      const filteredList = jobDetails.filter(item =>
  ...
  changeEmployType = id => {
    this.setState(
      prevState => ({
        employmentTypeInput: [...prevState.employmentTypeInput, id],
      }),
      this.getJobsDetails,
    )
  }
